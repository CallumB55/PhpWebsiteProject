...
<?php
	include('dbConnection.php');
	$roomInfoQ = "select ID from room where RoomType='".$_POST["roomtype"]."' and HotelID='".$_POST["hid"]."' limit 1";
	$roomInfo = mysqli_query($db, $roomInfoQ);
	while($room= mysqli_fetch_assoc($roomInfo)){
		$roomID = $room["ID"];
	}
	
	
	
	$room2CartQ = "Insert into cart (RoomID,CustomerEmail,startDate,StayDuration) Values (".$roomID.",'".$_COOKIE["customer"]."','".$_POST["startDate"]."',".$_POST["nightsStaying"].")";
	
	$room2Cart = mysqli_query($db, $room2CartQ);
