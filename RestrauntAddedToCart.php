...
<?php
	include('dbConnection.php');
	$restrauntInfoQ = "select ID from restraunt where MealType='".$_POST["MealType"]."' and HotelID='".$_POST["hid"]."' limit 1";
	$restrauntInfo = mysqli_query($db, $restrauntInfoQ);
	while($restraunt= mysqli_fetch_assoc($restrauntInfo)){
		$restrauntID = $restraunt["ID"];
	}
	
	
	
	$restraunt2CartQ = "Insert into cart (restrauntID,CustomerEmail,startDate) Values (".$restrauntID.",'".$_COOKIE["customer"]."','".$_POST["startDate"]."')";
	$restraunt2Cart = mysqli_query($db, $restraunt2CartQ);
