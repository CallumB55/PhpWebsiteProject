...
<?php
include('dbConnection.php');
$credentialChecker = "SELECT Email, FirstName FROM customer WHERE Email = '".mysqli_real_escape_string($db,$_POST["email"])."' and password ='".mysqli_real_escape_string($db,$_POST["password"])."'";

$credentialCheck = mysqli_query($db, $credentialChecker);
if(mysqli_num_rows($credentialCheck)==0){
	...
	while($row = mysqli_fetch_assoc($credentialCheck)){
